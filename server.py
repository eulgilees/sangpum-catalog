...
def init_tables():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS memo_comments (
        id SERIAL PRIMARY KEY,
    ...
    try:
        ...
        c2.execute("ALTER TABLE as_requests ADD COLUMN IF NOT EXISTS barcode TEXT DEFAULT ''")
        c2.execute("ALTER TABLE as_requests ADD COLUMN IF NOT EXISTS manager TEXT DEFAULT ''")
...
def add_order(data):
    ...
    c.execute('''INSERT INTO orders(barcode,name,qty,order_date,payment,ordered,pickup_date,
                 customer,phone,delivery,address,receptionist,staff,note,created_at,store,receipt_no) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id''',
              (data.get('barcode',''), data.get('name',''), data.get('qty',1),
               ...
               data.get('store',''), data.get('receipt_no','')))
...
def update_order(data):
    ...
    c.execute('''UPDATE orders SET qty=%s,order_date=%s,payment=%s,ordered=%s,pickup_date=%s,
                 customer=%s,phone=%s,delivery=%s,address=%s,receptionist=%s,staff=%s,note=%s,receipt_no=%s WHERE id=%s''',
              (data.get('qty',1), data.get('order_date',''), data.get('payment','미불'),
               ...
               data.get('receptionist',''), data.get('staff',''), data.get('note',''),
               data.get('receipt_no',''), data['id']))
    conn.commit(); release_db(conn)
...
def add_as_request(data):
    conn = data_db(); c = conn.cursor()
    c.execute('''INSERT INTO as_requests(received_date,product_name,content,customer,phone,delivery,staff,note,status,created_at,store,request_type,barcode,manager)
              ...
              (data.get('received_date',''), data.get('product_name',''), data.get('content',''),
               ...
               data.get('store',''), data.get('request_type','AS'), data.get('barcode',''),
               data.get('manager','')))
    new_id = c.fetchone()[0]; conn.commit(); release_db(conn); return new_id
...
def update_as_request(data):
    ...
    c.execute('''UPDATE as_requests SET received_date=%s,product_name=%s,content=%s,customer=%s,
                 phone=%s,delivery=%s,staff=%s,note=%s,request_type=%s,barcode=%s,manager=%s WHERE id=%s''',
              (data.get('received_date',''), data.get('product_name',''), data.get('content',''),
               data.get('customer',''), data.get('phone',''), data.get('delivery','없음'),
               data.get('staff',''), data.get('note',''), data.get('request_type','AS'),
               data.get('barcode',''), data.get('manager',''), data['id']))
    conn.commit(); release_db(conn)
...
def get_memo_comments(memo_id):
    conn = data_db(); c = conn.cursor()
    c.execute('SELECT * FROM memo_comments WHERE memo_id=%s ORDER BY id', (memo_id,))
    rows = rows_to_dicts(c); release_db(conn); return rows

def add_memo_comment(data):
    conn = data_db(); c = conn.cursor()
    c.execute('INSERT INTO memo_comments(memo_id,display_name,content,created_at) VALUES(%s,%s,%s,%s) RETURNING id',
              (data.get('memo_id'), data.get('display_name',''), data.get('content',''), data.get('created_at','')))
    new_id = c.fetchone()[0]; conn.commit(); release_db(conn); return new_id

def delete_memo_comment(cid):
    conn = data_db(); c = conn.cursor()
    c.execute('DELETE FROM memo_comments WHERE id=%s', (cid,))
    conn.commit(); release_db(conn)
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif parsed.path == '/api/memos/comments':
            mid = parsed.query.split('memo_id=')[-1].split('&')[0] if 'memo_id=' in parsed.query else None
            if mid: self.send_json(get_memo_comments(int(mid)))
            else: self.send_json([])
    ...
    def do_POST(self):
        ...
        if self.path == '/api/auth/register':
            ...
            self.send_json({'ok': True})
        elif self.path == '/api/memos/comment':
            self.send_json({'ok': True, 'id': add_memo_comment(body)})
        elif self.path == '/api/memos/comment/delete':
            delete_memo_comment(body['id']); self.send_json({'ok': True})
